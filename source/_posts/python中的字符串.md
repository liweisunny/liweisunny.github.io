...
方法1，使用简单直接，但是网上不少人说这种方法效率低，之所以说python 中使用 + 进行字符串连接的操作效率低下，是因为python中字符串是不可变的类型，使用 + 连接两个字符串时会生成一个新的字符串，生成**新的字符串就需要重新申请内存**，当连续相加的字符串很多时(a+b+c+d+e+f+...) ，效率低下就是必然的了
