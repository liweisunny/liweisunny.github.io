...
toc: true
---
  首先，学习python，掌握文件操作这一技能是必不可少的，本篇博文由浅入深，详细介绍python中操作文件的各种方法、各种方法之间的对比以及如何高效的读取文件等；Python中文件操作的三个步骤：
...
# 打开文件
Python中file()与open()两个都能够打开文件，对文件进行操作，也具有相似的用法和参数，并且返回的都是一个文件对象；但是，这两种文件打开方式有本质的区别，file是一个类，用file()来打开文件，相当于这是在构造文件类，而用open()打开文件，是用python的内建函数来操作，这是不一样的，另外python3已经不再支持file()了；So，<font color=#FF0000 size=5>**强烈建议使用open()函数**</font>。
## open()
...
4. buffering(可选):控制着文件的缓冲，
	1. 如果参数是0(或者是False)，I/O(输人/输出)就是无缓冲的(所有的读写操作都直接针对硬盘);
	2. 如果是1(或者是true), I/O就是有缓冲的(意味着Python使用内存来代替硬盘，让程序更快，只有使用flush或者close时才会更新硬盘上的数据;如果是大于1的数字代表缓冲区的大小(单位是字节)；
	3. 如果是-1(或者是任何负数)代表使用默认的缓冲区大小。
	4. 使用默认时，会采用下面策略来选择：
...
4. 'w'|写模式，会先清空文件内容
5. 'a'|追加模式
6. 'b'|二进制模式（可添加到其他模式中使用，如：'rb'：表示读取二进制文件）
7. '+'|读/写模式（可添加到其他模式中使用）
8. 'a+'|可读写模式，写只能写在文件末尾
9. 'w+'|可读写，与'a+'的区别是要清空文件内容(<font color=#FF0000 size=5>**不推荐使用**</font>)
10. 'r+'|可读写，与'a+'的区别是可以写到文件任何位置
...
2. 语法格式：file.seek(offset, whence=0):
	1. offset: 偏移量，需要向前或者是向后移动的字节数
	2. whence: 可选值，默认为0, 可选值为1或者2，表示从何处开始计算偏移，具体来说，0表示从当前位置开始计算偏移，1表示从文件头位置开始计算偏移，2表示从文件尾开始计算偏移。
### 代码演示
